 ...
 * value as variable {@link tempo}. If the input value is less than 20 or more than 240, tempo is set to
 * 20 or 240 beats per minute (bpm) respectively. The function also sets the tempo slider value to reflect the new input value.
 * @param {*} event
 * @returns {number}
 */
const submitHandler = (event) => {
    event.preventDefault();
    tempo = tempoInput.value < 20 ? 20 
        : tempoInput.value > 240 ? 240 
        : tempoInput.value;
...
/**
 * Plus button increases the tempo variable and counter by 1.
 * @returns {number} tempo
 */
const addHandler = () => {
    if (tempo >= 240) {
...
/**
 * Minus button decreases the tempo variable and counter by 1.
 * @returns {number} tempo
 */
...
/**
 * Function sets the counter to display the Italian name for the current tempo.
 */
const setTempoItalian = () => {
    tempoItalianName.innerText = `${
        (tempo >= 20 && tempo < 40) ? "Grave"
        : (tempo >= 40 && tempo < 60) ? "Largo"
        : (tempo >= 60 && tempo < 66) ? "Larghetto"
        : (tempo >= 66 && tempo < 76) ? "Adagio"
        : (tempo >= 76 && tempo < 108) ? "Andante"
        : (tempo >= 108 && tempo < 120) ? "Moderato"
        : (tempo >= 120 && tempo < 156) ? "Allegro"
        : (tempo >= 156 && tempo < 176) ? "Vivace"
        : (tempo >= 176 && tempo < 200) ? "Presto"
        : (tempo >= 200) ? "Prestissimo" : ""
    }`;   
}
